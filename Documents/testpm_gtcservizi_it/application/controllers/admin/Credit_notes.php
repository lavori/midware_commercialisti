...
class Credit_notes extends AdminController
{
    ...
    public function import_xml()
    {
        ...
        if (empty($_FILES['xml_files']['name'][0])) {
            set_alert('warning', _l('no_file_uploaded'));
            redirect(admin_url('credit_notes'));
        }

        $results  = $this->credit_notes_model->import_from_xml_bulk($_FILES['xml_files']);
        $imported = count(array_filter($results, function ($r) { return $r['success']; }));
        $failed   = count($results) - $imported;

        if ($imported > 0) {
            set_alert('success', sprintf(_l('credit_notes_xml_imported'), $imported));
        }
        if ($failed > 0) {
            set_alert('warning', sprintf(_l('credit_notes_xml_failed'), $failed));
        }
