...
class Credit_notes_model extends App_Model
{
    ...
    */
    ...
    /**
     * Import multiple FatturaPA XML files as credit notes.
     * @param  array $files  $_FILES['xml_files'] superglobal array
     * @return array         Results per file: ['success'=>bool,'message'=>string,'id'=>int|null]
     */
    public function import_from_xml_bulk($files)
    {
        $results    = [];
        ...
        for ($i = 0; $i < $file_count; $i++) {
            if ($files['error'][$i] === UPLOAD_ERR_OK) {
                $results[] = $this->import_from_xml($files['tmp_name'][$i], $files['name'][$i]);
            } else {
                $results[] = [
                    'success' => false,
                    'message' => 'Upload error for: ' . $files['name'][$i],
                    'id'      => null,
                ];
            }
        }
    ...
    /**
     * Parse a single FatturaPA XML file and create a credit note.
     * @param  string $tmp_path      Temp upload path
     * @param  string $original_name Original filename (for messages)
     * @return array  ['success'=>bool,'message'=>string,'id'=>int|null]
     */
    public function import_from_xml($tmp_path, $original_name)
    {
        $xmlContent = @file_get_contents($tmp_path);
        if ($xmlContent === false) {
            return ['success' => false, 'message' => 'Cannot read: ' . $original_name, 'id' => null];
        }
        ...
        if ($xml === false) {
            libxml_clear_errors();
            return ['success' => false, 'message' => 'Invalid XML: ' . $original_name, 'id' => null];
        }
        libxml_clear_errors();
        ...
        if (!$header || !$body) {
            return ['success' => false, 'message' => 'Unsupported XML format: ' . $original_name, 'id' => null];
        }

        // --- CessionarioCommittente (buyer → client in Perfex) ---
        $committente = $header->CessionarioCommittente;
        $vatCode     = (string)$committente->DatiAnagrafici->IdFiscaleIVA->IdCodice;
        $cf          = (string)$committente->DatiAnagrafici->CodiceFiscale;
        $clientName  = (string)$committente->DatiAnagrafici->Anagrafica->Denominazione;
        ...
        $vatToSearch = !empty($vatCode) ? $vatCode : $cf;
        $address = [
            'street'  => trim((string)$committente->Sede->Indirizzo . ' ' . (string)$committente->Sede->NumeroCivico),
            'city'    => (string)$committente->Sede->Comune,
            'state'   => (string)$committente->Sede->Provincia,
            'zip'     => (string)$committente->Sede->CAP,
            'country' => (string)$committente->Sede->Nazione,
        ];

        // --- CedentePrestatore (supplier → admin note) ---
        $prestatore   = $header->CedentePrestatore;
        $supplierName = (string)$prestatore->DatiAnagrafici->Anagrafica->Denominazione;
        if (empty(trim($supplierName))) {
            $supplierName = trim(
                (string)$prestatore->DatiAnagrafici->Anagrafica->Nome . ' ' .
                (string)$prestatore->DatiAnagrafici->Anagrafica->Cognome
            );
        }

        // --- Document header ---
        $docData   = $body->DatiGenerali->DatiGeneraliDocumento;
        $docDate   = (string)$docData->Data;
        $docNumber = (string)$docData->Numero;
        $docTotal  = (float)str_replace(',', '.', (string)$docData->ImportoTotaleDocumento);

        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $docDate)) {
            $docDate = date('Y-m-d');
        }

        // --- DatiRiepilogo → subtotal and tax ---
        $subtotal = 0.0;
        $totalTax = 0.0;
        foreach ($body->DatiBeniServizi->DatiRiepilogo as $riepilogo) {
            $subtotal += (float)str_replace(',', '.', (string)$riepilogo->ImponibileImporto);
            $totalTax += (float)str_replace(',', '.', (string)$riepilogo->Imposta);
        }

        if ($docTotal <= 0) {
            $docTotal = $subtotal + $totalTax;
        }

        // --- Line items from DettaglioLinee ---
        $items = [];
        $order = 1;
        foreach ($body->DatiBeniServizi->DettaglioLinee as $linea) {
            $aliquota    = (float)str_replace(',', '.', (string)$linea->AliquotaIVA);
            $description = (string)$linea->Descrizione;
            $qty         = (float)str_replace(',', '.', (string)$linea->Quantita);
            if ($qty <= 0) {
                $qty = 1;
            }
            $rate = (float)str_replace(',', '.', (string)$linea->PrezzoUnitario);
            $unit = (string)$linea->UnitaMisura;

            $taxname = [];
            if ($aliquota > 0) {
                $taxname[] = $this->_get_tax_name_by_rate_cn($aliquota);
            }
        ...
        if (empty($items)) {
            return ['success' => false, 'message' => 'No line items in: ' . $original_name, 'id' => null];
        }

        // --- Lookup or auto-create client ---
        $clientId = $this->_get_or_create_client_xml($vatToSearch, $clientName, $address, $cf);
        if (!$clientId) {
            return ['success' => false, 'message' => 'Could not resolve client for: ' . $original_name, 'id' => null];
        }

        $currencyId = $this->_get_currency_id_xml('EUR');
        ...
        $data = [
            ...
            'adjustment'       => 0,
            'currency'         => $currencyId,
            'adminnote'        => $supplierName,
            'newitems'         => $items,
        ];

        $id = $this->add($data);
        if ($id) {
            return ['success' => true, 'message' => 'Imported: ' . $original_name, 'id' => $id];
        }

        return ['success' => false, 'message' => 'DB error saving: ' . $original_name, 'id' => null];
    }

    /**
     * Look up a client by VAT / company name. Creates a new client if not found.
     */
    private function _get_or_create_client_xml($vat, $name, $address, $cf = '')
    {
        // 1) Match by P.IVA
        if (!empty($vat)) {
        ...
        // 2) Match by Codice Fiscale (if different from P.IVA)
        if (!empty($cf) && $cf !== $vat) {
            $this->db->where('vat', $cf);
            $client = $this->db->get(db_prefix() . 'clients')->row();
            if ($client) {
                return (int)$client->userid;
            }
        ...
        // 4) Auto-create new client
        $countryId = 0;
        if (!empty($address['country'])) {
            $countryRow = $this->db->get_where(
                db_prefix() . 'countries',
                ['iso2' => strtoupper($address['country'])]
            )->row();
            if ($countryRow) {
                $countryId = (int)$countryRow->country_id;
            }
        }

        $this->db->insert(db_prefix() . 'clients', [
            'company'         => !empty($name) ? $name : ('Cliente ' . $vat),
            'vat'             => $vat,
        ...
        if ($newId) {
            log_activity('New Client Created via XML Import [ID: ' . $newId . ']');
            return (int)$newId;
        }
    ...
    /**
     * Returns "taxname|taxrate" string for a given rate.
     * Auto-creates the tax record if not found.
     */
    private function _get_tax_name_by_rate_cn($rate)
    {
        $rate = round((float)$rate, 2);
        $this->db->where('taxrate', $rate);
    ...
    /**
     * Returns currency ID by symbol (e.g. 'EUR'). Falls back to default currency.
     */
    private function _get_currency_id_xml($symbol)
    {
        $this->db->where('name', $symbol);
        $currency = $this->db->get(db_prefix() . 'currencies')->row();
        if ($currency) {
            return (int)$currency->id;
        }

        $base = $this->db->get_where(db_prefix() . 'currencies', ['isdefault' => 1])->row();
