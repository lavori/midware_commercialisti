                                                ...
                                                <div class="form-control-wrap">
                                                    <div class="input-group">
                                                        <?php if (isset($exam['data_scritto']) && empty($exam['data_scritto']) && empty($exam['voto'])) { ?>
                                                            <a onClick="apriModal('Fissa Data Scritto','Gestione Esame','scadenzaPacchetti/form_data','<?php echo $exam['id'] . '|' .$pacchetto. '|' . 'scritto'; ?>')"
                                                    ...
                                                    <div class="input-group">
                                                        <?php if (empty($exam['voto']) && isset($exam['data_scritto']) && !empty($exam['data_scritto'])) { ?>
                                                            <?php if (isset($exam['data_orale']) && empty($exam['data_orale'])) { ?>
                                                                <a onClick="apriModal('Fissa Data Orale','Gestione Esame','scadenzaPacchetti/form_data','<?php echo $exam['id'] . '|' .$pacchetto. '|orale|' . $exam['data_scritto']; ?>')"
                                                                    class="btn btn-primary">Fissa</a>
                                                            <?php } else { ?>
                                                                <label class="form-label mb-0 ms-3">
                                                                    <?php
                                        ...
                                        <?php if (isset($exam['data_scritto']) && !empty($exam['data_scritto'])) { ?>
                                            <?php if (!isset($exam['voto']) || empty($exam['voto'])) { ?>
                                                <div class="col-lg-4 d-flex align-items-center">
                                                    <a onClick="apriModal('Modifica','Gestione Esame','scadenzaPacchetti/modificaEsame','<?php echo $exam['id'] . '|' .$pacchetto. '|' . $exam['data_scritto'] . '|' . $exam['data_orale'] . '|' . $TE; ?>')"
                                                        class="btn btn-primary me-2">Modifica</a>
                                                    <a onClick="apriModal('Inserisci Voto','Gestione Esame','scadenzaPacchetti/insertVoto','<?php echo $exam['id']. '|' .$pacchetto ?>')"
