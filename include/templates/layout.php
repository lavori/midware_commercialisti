    ...
    <script>
        ...
        async function apriModal(a, b, c, d = 'new', table = '') {
            ...
            try {
                ...
                // *** Inizializza Select2 sugli elementi appena caricati ***
                $('#contentModal select').select2();

                // *** Inizializza Datepicker sugli elementi appena caricati ***
                // Cerca la funzione di inizializzazione del datepicker del tema Dashlite
                if (typeof NioApp !== 'undefined' && typeof NioApp.components !== 'undefined' && typeof NioApp.components.nioDatepicker === 'function') {
                     NioApp.components.nioDatepicker(); // Chiama la funzione di inizializzazione del tema
                } else {
                    // Fallback: inizializzazione manuale se la funzione NioApp non è disponibile
                    // Questo dipende dalla libreria datepicker specifica usata (spesso Bootstrap-datepicker)
                    $('#contentModal .date-picker-alt').each(function() {
                         $(this).datepicker({
                             format: $(this).data('date-format') || 'yyyy-mm-dd', // Usa il formato specificato nel data-attribute
                             autoclose: true, // Esempio di opzione
                             todayHighlight: true // Esempio di opzione
                             // Aggiungi altre opzioni del datepicker se necessario
                         });
                    });
                }

                // *** Inizializza DataTable sugli elementi appena caricati (if table == 1) ***
                // Rimosso il wrapper $(document).ready() perché non necessario per contenuti dinamici
                if (table == 1) {
                    // DataTable Init @v1.0
                    $('#contentModal .datatable-init').DataTable({
                        responsive: {
                            details: true
                        }
                    });
                    // Target la tabella *all'interno* del contenuto del modale
                    $('#contentModal .datatable-init-export').DataTable({
                        responsive: {
                            details: true
                        },
                        buttons: ['copy', 'excel', 'csv', 'pdf']
                        //dom: "<'row justify-content-between g-2'<'col-7 col-sm-4 text-start'f><'col-5 col-sm-8 text-end'lB>><'datatable-wrap my-3't><'row align-items-center'<'col-7 col-sm-12 col-md-9'p><'col-5 col-sm-12 col-md-3 text-start text-md-end'i>>"
                    });
                    // Questa riga sembra una configurazione globale, potrebbe non essere necessaria qui
        ...
        <!-- traduzione datatable -->
        // Oggetto di traduzione per DataTables in italiano
        var dataTableItalian = {
            ...
            "buttons": { // Traduzioni per i bottoni di DataTables (se usi l'estensione Buttons)
                "copy": "Copia",
                "excel": "Excel",
                "csv": "CSV",
                "pdf": "PDF",
        ...
        // Applica globalmente le traduzioni a DataTables
        // Assicurati che questo codice venga eseguito dopo il caricamento di jQuery e DataTables
        // e prima dell'inizializzazione delle tue tabelle.
