<?php
     ...
     return function ($router)  use ($database, $dominio, $titolo, $apps, $menu, $dominioapi) {
        ...
        $router->addRoute('/users', function () use ($database, $dominio, $titolo, $apps, $menu)  {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                if (isset($_GET['action']) && $_GET['action'] === "delete") {
                    $database->update('users', array('status' => 'disattivo'), 'id=' . $_GET['id']);
                }
            }
            $query_utenti  ="SELECT 
                                u.`id`,
        ...
        $router->addRoute('/users/tassisti', function () use ($database, $dominio, $titolo, $apps, $menu) {
            ...
            if (isset($_POST['action']) && $_POST['action']=='update') {
                ...
                // Conversione della data dal formato dd-mm-yyyy a YYYY-MM-DD
                $dataAssunzioneDatabase = null;
                if (isset($_POST['data']) && !empty($_POST['data'])) {
                    $dataOggetto = DateTime::createFromFormat('d-m-Y', $_POST['data']);
                    if ($dataOggetto) {
                        $dataAssunzioneDatabase = $dataOggetto->format('Y-m-d');
                    }
                }
                $data = array(
                    ...
                    'Nome' => $_POST['nome'],
                    'Cognome' => $_POST['cognome'],
                    'LicenzaDiGuida'=> $_POST['licenza'],
                    'Telefono'=> $_POST['tel'],
                    'Email'=> $_POST['email'],
                    'TargaTaxi'=> $_POST['targa'],
                    'TurniDiLavoro'=> $giorni_serializzati,
                    'DataAssunzione'=> $dataAssunzioneDatabase,
                    'Visibilita' => 1,
                );
            ...
            $tassisti = $database->select("tassisti", "id, Azienda, Nome, Cognome, Email, TargaTaxi", $wheretassisti);
        ...
        $router->addRoute('/corrispettivi/taxisti', function () use ($database, $dominio, $titolo, $apps, $menu)  {
            ...
            $query="SELECT 
                        ...
                        COUNT(t.id) AS numero_tassisti 
                    FROM aziende a 
                    LEFT JOIN tassisti t ON a.id = t.Azienda AND t.Visibilita = 1 
                    WHERE a.attiva = 'Si' 
                    GROUP BY a.id, a.ragione_sociale 
                    ORDER BY nome_azienda";
            $aziende=$database -> query($query);
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                //echo"<pre>"; print_r($_POST); echo "</pre>"; exit();
                if (isset($_POST['tipo_action']) && $_POST['tipo_action']=="ricerca"){
                    $at = isset($_POST['taxista']) ? substr($_POST['taxista'], 0, 1) : '';
                    if ($at=='A'){
                       $id_azienda=substr($_POST['taxista'],1); 
                    }elseif($at=='T'){
                        $id_taxista=substr($_POST['taxista'],1);
                    }
                    ...
                    // --- Costruzione Query ---
                    $queryParams = [];
                    if ($at === 'A') { // Ricerca Corrispettivi (aggregati)
                        $query = "  SELECT 
                                        ct.contabilizzato, 
                                        ct.n_registrazione, 
                                        ct.data, 
                                    ...
                                    LEFT JOIN tassisti t ON it.tassista_id = t.id  
                                    WHERE t.Azienda = ".$id_azienda." ";
                        ...
                        $query = "SELECT it.data_incasso AS data, DAYNAME(it.data_incasso) AS giorno_settimana, it.valore_incasso, ct.contabilizzato AS incasso_contabilizzato
                                  ...
                                  WHERE it.tassista_id = '$id_taxista'";
        ...
        $router->addRoute('/corrispettivi/ridistribuisci-incassi', function () use ($database, $dominio, $titolo, $apps, $menu) {
            ...
            if( $_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $azienda = $_POST['azienda'];
                //echo $azienda; exit(); 
                if(checkIncassiCorrispettiviEsistono($database, $meseRiferimento, $annoRiferimento, $azienda)){
                    $result_azione="Corrispettivi già generati per il mese di ".$meseRiferimento."/".$annoRiferimento;
                    $tipo_alert="danger";
                } else {
                    $azione = ridistribuisciIncassiRoute($database, $meseRiferimento, $annoRiferimento, $incassoMensile, $azienda);
                ...
                $corrispettivi = getCorrispettiviPerMese($database, $meseRiferimento, $annoRiferimento, $azienda); 
            } 
        ...
        $router->addRoute('/corrispettivi/invio-corrispettivi', function () use ($database, $dominio, $dominioapi, $titolo, $apps, $menu) {
            if( $_SERVER['REQUEST_METHOD'] === 'POST') :
                ...
                //inserire codilogica di recupero del $codiceAziendaGestionale
                ...
                if (!isset($_POST['corrispettivo']) || !is_array($_POST['corrispettivo']) || empty($_POST['corrispettivo'])) {
                    ...
                    foreach($_POST['corrispettivo'] as $key => $value) {
                        ...
                        try {
                            ...
                            $risposta_array = scritturaPrimaNota($database, $cont, $id_azienda, $data_formatted, $tipo, $dnsapi,$codiceAziendaGestionale);
    ...
    function checkIncassiCorrispettiviEsistono(Database $database, int $meseRiferimento, int $annoRiferimento, $idazienda): bool {
        ...
        // Controllo nella tabella 'incassi tassisti'
        $queryIncassi = "SELECT COUNT(it.id) AS conteggio 
                         FROM incassi_taxi it
                         INNER JOIN tassisti t ON it.tassista_id = t.id
                         WHERE it.data_incasso BETWEEN '$primoGiorno' AND '$ultimoGiorno' AND t.Azienda = $idazienda";
                        
        $resultIncassi = $database->query($queryIncassi);
        if ($resultIncassi && isset($resultIncassi[0]['conteggio']) && $resultIncassi[0]['conteggio'] > 0) {
            return true; // Esistono già incassi per questo periodo
        }

        // Controllo nella tabella 'corrispettivi'
        $queryCorrispettivi = "SELECT COUNT(DISTINCT ct.data) AS conteggio
                               FROM corrispettivi_taxi as ct
                               WHERE ct.azienda = $idazienda AND 
                               ct.data BETWEEN '$primoGiorno' AND '$ultimoGiorno'";
                                 echo $queryCorrispettivi; // Debug
                                 // exit();       
        $resultCorrispettivi = $database->query($queryCorrispettivi);
        if ($resultCorrispettivi && isset($resultCorrispettivi[0]['conteggio']) && $resultCorrispettivi[0]['conteggio'] > 0) {
    ...
    function ridistribuisciIncassiRoute(Database $database, int $meseRiferimento, int $annoRiferimento, float $incassoMensile, int $idazienda) {
        ...
        try {
            ...
            $tassisti = $database->select("tassisti", ["id", "`TurniDiLavoro`", "Dimissioni"], $where = "Azienda = $idazienda");
    
            foreach ($tassisti as $tassista) {
                $tassistaId = $tassista['id'];
                // Decodifica dei turni
                $turni = unserialize($tassista['TurniDiLavoro']);
                $dataDimissioni = $tassista['Dimissioni']; // YYYY-MM-DD o NULL
                $turni = [];
                try {
                    // Aggiungi @ per sopprimere warning se unserialize fallisce
                    $decodedTurni = @unserialize($tassista['TurniDiLavoro']);
                    if ($decodedTurni !== false && is_array($decodedTurni)) {
                        $turni = $decodedTurni;
                    } else {
                         error_log("ATTENZIONE: Impossibile decodificare i turni per il tassista $tassistaId. Valore: " . $tassista['TurniDiLavoro']);
                ...
                for ($i = 1; $i <= $numeroGiorni; $i++) {
                    ...
                    // 4. Il tassista non è dimesso o la data è precedente alle dimissioni?
                    $isActiveForDate = true; // Assumiamo attivo di default
                    if (!empty($dataDimissioni)) {
                        // Confronta le date come stringhe (funziona per il formato YYYY-MM-DD)
                        // o converti in oggetti DateTime per un confronto più robusto se necessario.
                        if ($dataFormattata >= $dataDimissioni) {
                    ...
                    if ($isInShift && $isWeekday && !$isHoliday && $isActiveForDate) {
                ...
                $numeroGiorniEffettivamenteLavoratiNelMese = count($giorniLavorativiTassista);

                if ($numeroGiorniEffettivamenteLavoratiNelMese > 0) {
                    // Determina su quanti giorni basare il calcolo dell'incasso medio giornaliero
                    $giorniSuCuiMediareIncasso = $numeroGiorniEffettivamenteLavoratiNelMese; // Default: giorni effettivi

                    if (!empty($dataDimissioni)) { // Se il tassista ha una data di dimissioni
                        // Per i tassisti con data di dimissioni, l'incasso mensile target
                        // viene mediato su un numero convenzionale di 22 giorni per calcolare l'incasso medio di riferimento.
                        $giorniSuCuiMediareIncasso = 22;
                    }
                    $incassoMedioGiornaliero = $incassoMensile / $giorniSuCuiMediareIncasso;
                } else {
                    $incassoMedioGiornaliero = 0; // Nessun giorno lavorativo, nessun incasso medio da calcolare
                }
                ...
                foreach ($giorniLavorativiTassista as $dataLavorativa) {
                    ...
                    // Assicura che l'incasso non sia negativo e arrotonda al decimo
                    $incassoGiornaliero = max(0, round($incassoMedioGiornaliero * (1 + $variazione), 1));      
            ...
            if (!empty($incassiGiornalieri)) {
                ...
                foreach ($incassiGiornalieri as $data => $valore) {
                    if ($valore >= 0) { // Inserisci anche 0 se necessario
                        ...
                        $valuesCorrispettivi[] = "('$escapedData', '$idazienda', '$escapedGiornoNome', $escapedValore)";
                    }
                }
 
                if (!empty($valuesCorrispettivi)) {
                    $queryCorrispettivi = "INSERT INTO corrispettivi_taxi (data, azienda, giorno_settimana, valore_corrispettivo) VALUES ";
    ...
    function getCorrispettiviPerMese(Database $database, int $meseRiferimento, int $annoRiferimento, int $idazienda) {
        ...
        $query = "SELECT
                  ...
                  WHERE ct.azienda='$idazienda' AND ct.data BETWEEN '$primoGiorno' AND '$ultimoGiorno' 
                  ORDER BY ct.data";
